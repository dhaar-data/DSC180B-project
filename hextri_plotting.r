...
setwd('your_path_here') # set your working directory path here

# Plotting estimators
estimators <- read.csv(file = "results/inference/estimators.csv", header = TRUE)
...
hextri(`estimate with predicted outcome`~`estimate with true outcome`, 
       ...
       style='size', nbins=10, xlim=c(-5, 10), ylim=c(-5, 10))
abline(0,1)
...
dev.copy(png,filename="results/figures/estimators.png")
dev.off()

# Plotting standard errors
ses <- read.csv(file = "results/inference/ses.csv", header = TRUE)
...
hextri(`standard error with predicted outcome`~`standard error with true outcome`, 
       ...
       style='size', nbins=15, xlim=c(0,2.5), ylim=c(0,2.5))
abline(0,1)
legend('topleft', col=rev(c('blue', 'lightblue', 'orange')), pch=15, 
       legend=rev(c('Parametric Bootstrap Postpi', 
                    'Non-Parametric Bootstrap Postpi','No Correction')),
...
dev.copy(png,filename="results/figures/ses.png")
dev.off()

# Plotting t-statistic
tstat <- read.csv(file = "results/inference/t_stat.csv", header = TRUE)
...
hextri(`statistic with predicted outcome`~`statistic with true outcome`, 
       ...
       style='size', nbins=15, xlim=c(-2,15), ylim=c(-2,15))
abline(0,1)
legend('topleft', col=rev(c('blue','lightblue','orange')), pch=15, 
       legend=rev(c('Parametric Bootstrap Postpi', 
                    'Non-Parametric Bootstrap Postpi','No correction')),
...
dev.copy(png,filename="results/figures/t_stat.png")
dev.off()
